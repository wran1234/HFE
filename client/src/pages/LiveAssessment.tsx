...
import { ChevronDown, ChevronUp, Info, Lightbulb, ShieldCheck } from "lucide-react";
...
import { getRiskLabel, loadProfile } from "../lib/userProfile";
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 className="text-2xl font-bold text-white">Live Guided Assessment</h1>
            </div>
            <p className="text-slate-400 text-sm">
              Assessing home for <span className="text-white font-medium">{subject}</span>
              {profile.age > 0 && <span className="text-slate-500"> · Age {profile.age}</span>}
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className={`text-sm font-semibold ${riskInfo.color}`}>{riskInfo.label}</div>
            <button onClick={() => navigate("/assessment")} className="btn-secondary py-2 px-3 text-xs">
              Use photo assessment
            </button>
          </div>
        </div>
        ...
        <div className="mb-5">
          ...
          >
            ...
            <span className="text-sm font-medium text-brand-300 flex-1">Live walkthrough tips</span>
            {tipsOpen ? <ChevronUp className="w-4 h-4 text-brand-400" /> : <ChevronDown className="w-4 h-4 text-brand-400" />}
          </button>
          {tipsOpen && (
        ...
        <div className="flex items-start gap-3 p-3.5 bg-slate-900 border border-slate-700 rounded-xl mb-6 text-sm text-slate-400">
          ...
          <p>
            <span className="font-medium text-slate-300">Optional mode:</span> live video requires camera, microphone, and a stable connection. Photo assessment is the default recommended flow.
          </p>
        </div>
