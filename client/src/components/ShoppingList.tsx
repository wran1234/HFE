...
const PRODUCT_MAP: Record<string, Product[]> = {
  ...
  "Accessibility": [
    { name: "Door Offset Hinges (gain 2\" clearance, pair)", priceRange: "$20–$35", category: "Accessibility", isDIY: true, forHazard: "narrow_doorway" },
    { name: "Door Lever Handle Replacement Set", priceRange: "$35–$60", category: "Accessibility", isDIY: true, forHazard: "narrow_doorway" },
  ],
  "Outdoor Safety": [
    { name: "Anti-Slip Tape for Outdoor Steps (30 ft)", priceRange: "$12–$25", category: "Outdoor Safety", isDIY: true, forHazard: "slippery_outdoor_steps" },
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-warm-200 rounded-2xl p-6 shadow-sm">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="ml-auto text-xs text-warm-400">Available at hardware / home stores</span>
        </div>
        <div className="space-y-2">
          {relevantProducts.filter(p => p.isDIY).map((p, i) => (
            <div key={i} className="flex items-center justify-between p-3 bg-warm-50 border border-warm-200 rounded-xl">
      ...
      <p className="text-xs text-warm-400 text-center">
        Prices are estimates. Search product names on Amazon, Home Depot, Lowe's, or medical supply stores.
      </p>
    </div>
  );
}
